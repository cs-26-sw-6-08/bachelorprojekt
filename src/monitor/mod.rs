...
use crate::{
    ...
    monitor::{
        ...
        streams::{IoTStream, OutputStream},
    },
    program::Program,
};
use std::error::Error;
...
impl Program {
    pub async fn monitor(
        ...
        let Some(streams) = &mut self.environment else {
            return Err(errors::Error::FieldNotPresent.into());
        };
        let Some(size) = self.iotstream_len else {
            return Err(errors::Error::FieldNotPresent.into());
        };
        ...
        loop {
            ...
            #[cfg(debug_assertions)]
            {
                ...
                let colored_time = if elapsed.as_millis() > time_interval as u128 {
                    format!("{:?}", elapsed).red().bold()
                } else {
                    format!("{:?}", elapsed).bright_green().bold()
                };
    ...
    pub fn monitor_logic<'a>(
        env: &'a mut [OutputStream],
        t: &'a i128,
        device_stream: &'a IoTStream,
    ) -> Box<dyn Iterator<Item = MonitorElement> + 'a> {
        Box::new(
            env.iter_mut()
                .enumerate()
