...
impl Expr {
    pub fn soundness_check(&self) -> Result<(), Box<dyn Error>> {
        self.internal_soundness_check(false, false)
    }

    fn internal_soundness_check(&self, inside_num_aggregate: bool, inside_aggregate: bool) -> Result<(), Box<dyn Error>> {
        match self {
            ...
            Expr::Always { not: false, expr, .. } => {
                expr.internal_soundness_check(inside_num_aggregate, inside_aggregate)?;
            ...
            Expr::BinaryOperations { lhs, rhs, .. } => {
                lhs.internal_soundness_check(inside_num_aggregate, inside_aggregate)?;
                rhs.internal_soundness_check(inside_num_aggregate, inside_aggregate)?;
                Ok(())
            },
            Expr::UnaryOperations { operand, operator:_ } => {
                operand.internal_soundness_check(inside_num_aggregate, inside_aggregate)?;
                Ok(())
            },
            Expr::Function { aggregate_type, expr, bound: _ } => {
                if (aggregate_type.eq(&FunctionType::Sumtime) || aggregate_type.eq(&FunctionType::Avgtime) || aggregate_type.eq(&FunctionType::Counttime)) && inside_aggregate {
                    ...
                    expr.internal_soundness_check(inside_num_aggregate, true)?;
                } else {
                    expr.internal_soundness_check(true, true)?;
                }
                Ok(())
            },
